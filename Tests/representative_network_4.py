...
def main(repeater_delay_multiplier, avg_send_time, total_sim_packets):
    ...
    gw1 = sim.node(env, 2.00*d, 3.00*d, "gw")
    gw2 = sim.node(env, 4.00*d, 1.00*d, "gw") #isolated for label consistency

    repeaters.append(node(env, 2.00*d, 2.00*d, "rp")) #3
    repeaters.append(node(env, 1.00*d, 2.00*d, "rp")) #4
    repeaters.append(node(env, 1.20*d, 1.50*d, "rp")) #5
    repeaters.append(node(env, 2.20*d, 1.50*d, "rp")) #6
    repeaters.append(node(env, 2.20*d, 2.50*d, "rp")) #6

    no_of_repeaters = len(repeaters)

    #End devices
    enddevices.append(node(env, 0.80*d, 2.10*d, "ed"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("-total_sim_packets"         , default=10 , help="Total number of packets to process in the simulation")
